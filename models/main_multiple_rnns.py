...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--num_rnns', default=DEFAULT_NUM_RNNS, 
        help='number of rnns'
    )
    ...
    start_time = str(args.num_rnns) + '_' + datetime.now().strftime('%Y_%m_%d_%H_%M%S')
    if not args.feature_file:
        for i in range(int(args.num_rnns)):
            rnns[i] = Emb_RNNLM(rnn_params)
        ...
        for i in range(int(args.num_rnns)):
            rnns[i] =  Feature_RNNLM(rnn_params, feature_table)
        print('Fitting feature model...')

    train_lm(training, dev, rnn_params, rnns, ix2phone, start_time)
    for i in range(int(args.num_rnns)):
        rnns[i].eval()

    get_probs(args.test_file, rnns, phone2ix, args.judgement_file, device)
